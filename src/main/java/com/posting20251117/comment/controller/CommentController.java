package com.posting20251117.comment.controller;

import com.posting20251117.comment.dto.*;
import com.posting20251117.comment.service.CommentService;
...
public class CommentController {
    ...
    // 3-Layer-Architecture, 하위 레이어 포함시키기
    private final CommentService commentService;

    @PostMapping("/postings/{postingId}/comments")
    public ResponseEntity<CreateCommentResponse> saveComment(@PathVariable Long postingId, @RequestBody CreateCommentRequest request) {
    ...
    public ResponseEntity<List<GetCommentResponse>> getComments(@PathVariable Long postingId) { // @RequestParam(required=false) String xxx, Null일수도 있음
        return ResponseEntity.status(HttpStatus.OK).body(commentService.getAll(postingId)); // commentService.search(xxx)
    }
